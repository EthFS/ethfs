...
library KernelLib {
  ...
  function stat(KernelArea storage self, bytes calldata path) external view returns (FileSystem.FileType fileType, uint16 mode, uint ino, uint links, address owner, address group, uint entries, uint size, uint lastModified) {
    UserArea storage u = self.userArea[tx.origin];
